...
def verify_solution(T_desired, q_solution, link_params):
    ...
    p_desired = T_desired[0:3, 3]
    p_check = T_check[0:3, 3]

    R_desired = T_desired[0:3, 0:3]
    R_check = T_check[0:3, 0:3]

    position_error = np.linalg.norm(p_desired - p_check)
    orientation_error_val = np.linalg.norm(
        orientation_error(R_check, R_desired)
    )

    print("\n--- IK Verification ---")
    print("Position Error =", position_error)
    print("Orientation Error =", orientation_error_val)

    if position_error < 1e-4 and orientation_error_val < 1e-4:
        print("\nIK Solution Valid")
    else:
        print("\nIK Solution Invalid")
